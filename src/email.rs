...
/// re-parse from disk.
#[derive(Debug, Clone)]
#[allow(dead_code)] // fields read by tests + future cid: resolution bead
...
impl Email {
    ...
    fn extract_attachments(&mut self, message: &Message) -> Result<()> {
        ...
        while let Some(part) = message.attachment(index) {
            ...
            // Parts with a Content-ID are referenced from HTML bodies
            // via `cid:` — route them to `inline_images` regardless of
            // whether mail-parser tags them `InlineBinary` or `Binary`.
            // The cid is the only signal that an HTML body might want
            // to embed this part.
...
mod tests {
    ...
    fn parse_body_multipart_alternative_keeps_both_parts() {
        ...
        assert_eq!(
            email.body_plain.as_deref().map(str::trim_end),
            Some("plain rendition"),
            "text/plain part must populate body_plain",
        );
        let html = email
            .body_html
            .as_deref()
